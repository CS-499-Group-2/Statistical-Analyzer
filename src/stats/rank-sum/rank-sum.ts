...
export const RankSumOperation: TypedOperation<Record<string, never>> = {
  name: "Rank Sum Test",
  type: "Typed",
  isValid: columns => columns.length == 2,
  onSelected: selectedCellsByColumn => {
    const values = selectedCellsByColumn.map(column => column.values);
    const rankSum = wilcoxonRankSum(values[0], values[1]);
    const result: Result = {
      name: "Rank Sum Test | Columns: " + selectedCellsByColumn.map(column => column.name).join(","),
      values: [rankSum],
      graphs: [],
    };
    return [result];
